...
public interface SchBlockMapper {
    ...
    default List<SchBlock> DTOs2DBs(List<SchBlockDTO> dList, @Context CorChannel corChannel) {
    ...
    default void schBlockDTOToSchBlockAfterMapping(SchBlockDTO dto, @MappingTarget SchBlock entity, @Context CorChannel corChannel) {
        ...
        if (dto.getBlocks() != null && !dto.getBlocks().isEmpty()) {
            schBlocks = dto.getBlocks().stream().map(schBlockDTO -> new SchBlockSchBlock().child(this.DTO2DB(schBlockDTO, corChannel)).parent(entity).sequence(schBlockDTO.getSequence())).collect(toList());
        }
