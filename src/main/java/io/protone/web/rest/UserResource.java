 ...
 *
 * <p>This class accesses the User entity, and needs to fetch its collection of authoritiesf.</p>
 * <p>
 ...
 * <ul>
 * <li>We want to keep a lazy association between the user and the authoritiesf, because people will
 * quite often do relationships with the user, and we don't want them to get the authoritiesf all
 * the time for nothing (for performance reasons). This is the #1 goal: we should not impact our users'
 * application because of this use-case.</li>
 * <li> Not having an outer join causes n+1 requests to the database. This is not a real issue as
 * we have by default a second-level cache. This means on the first HTTP call we do the n+1 requests,
 * but then all authoritiesf come from the cache, so in fact it's much better than doing an outer join
