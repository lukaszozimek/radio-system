...
public class LibLabelResourceImpl implements LibLabelResource {
    ...
    @Override
    public ResponseEntity<LibLabelDTO> updateLabelUsingPUT(@ApiParam(value = "networkShortcut", required = true) @PathVariable("networkShortcut") String networkShortcut,
        ...
        if (libLabelDTO.getId() == null) {
            return createLabelUsingPOST(networkShortcut, libLabelDTO);
        }
    ...
    @Override
    public ResponseEntity<List<LibLabelDTO>> getAllLabelsUsingGET(@ApiParam(value = "networkShortcut", required = true) @PathVariable("networkShortcut") String networkShortcut,
                                                                  @ApiParam(value = "pagable", required = true) Pageable pagable) {
        log.debug("REST request to get all LibLabelDTO");
        Slice<LibLabel> libraries = libLabelService.findLabels(networkShortcut, pagable);
    ...
    public ResponseEntity<LibLabelDTO> createLabelUsingPOST(@ApiParam(value = "networkShortcut", required = true) @PathVariable("networkShortcut") String networkShortcut,
        ...
        LibLabel resultDB = libLabelService.createOrUpdateLabel(entity);
        LibLabelDTO labelDAO = libLabelMapper.DB2DTO(resultDB);
        return ResponseEntity.created(new URI("/api/v1/network/" + networkShortcut + "/label/label/" + labelDAO.getId()))
                .body(labelDAO);
    }

    @Override
    public ResponseEntity<Void> deleteLabelUsingDELETE(@ApiParam(value = "networkShortcut", required = true) @PathVariable("networkShortcut") String networkShortcut,
                                                       @ApiParam(value = "id", required = true) @PathVariable("id") Long id) {
        log.debug("REST request to delete LIBLabel : {}", id);
        libLabelService.deleteLabel(id, networkShortcut);
        return ResponseEntity.ok().build();
    }


    @Override
    public ResponseEntity<LibLabelDTO> getLabelUsingGET(@ApiParam(value = "networkShortcut", required = true) @PathVariable("networkShortcut") String networkShortcut,
                                                        @ApiParam(value = "id", required = true) @PathVariable("id") Long id) {
        log.debug("REST request to get label: {}", id);
        LibLabel label = libLabelService.findLabel(networkShortcut, id);
