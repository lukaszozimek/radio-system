...
class Deck:
    ...
    def play(self):
        ...
        self.thd = threading.Thread(target=self.worker)
        self.thd.start()
    ...
    def eofEvent(self):
        if self.mixFired:
            ...
            print '[{0}] ON EOFMIX /{1}/{2}/{3}'.format(self.name, self.item.network, self.item.library, self.item.idx)
            self.mixDeck.play()
            self.load()
        self.eofFired = True
    ...
    def errEvent(self):
        ...
        if self.mixFired == False and self.eofFired == False:
            print '[{0}] ON ERRMIX /{1}/{2}/{3}'.format(self.name, self.item.network, self.item.library, self.item.idx)
            self.mixDeck.play()
            self.load()
