package io.protone.library.constans;

/**
 * Created by lukaszozimek on 28/03/2017.
 */
public final class MarkerConstans {
    public static final String TERs = "TERs";
    public static final String TERe = "TERe";
    public static final String SEGs = "SEGs";
    public static final String SEGe = "SEGe";
    public static final String INT = "INT";
    public static final String AUDs = "AUDs";
    public static final String AUDe = "AUDe";
    public static final String EMPTY_TIMER = "EMPTY_TIMER";

    private MarkerConstans() {
    }

}
